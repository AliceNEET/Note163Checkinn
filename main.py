...
if __name__ == "__main__":
    ...
    _check_item = config.get("YOUDAO", [])[0]
    pushToken = config.get("PUSHTOKEN", [])
    if pushToken != None:
        msg1 = YouDao(check_item=_check_item).main()
        content = "今日有道云笔记签到！\n"+msg1
        requests.post('https://www.pushplus.plus/send', { 'token': pushToken, 'title': '有道云笔记签到', 'content': content });
